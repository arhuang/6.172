...
# Parse stdout from mdriver when executed on Azure worker machines.
def parse_cqrun_stdout(stdout):
    result = {}
    for line in stdout.splitlines():
...
class MdriverTuner(MeasurementInterface):
  ...
  def run(self, desired_result, input, limit):
    ...
    cqrun = self.args.cqrun
    if cqrun:
        print "Running on cloud queue worker machines..."
    else:
    ...
    bin_cmd = ''
    if cqrun:
        bin_cmd = 'cqrun ./mdriver -g ' + trace_params
    else:
    ...
    if cqrun:
        # Parse the output.
        result = parse_cqrun_stdout(run_result['stdout'])
...
if __name__ == '__main__':
  ...
  argparser.add_argument('--cqrun', action='store_true',
                         help='run on cloud worker machines instead of local')
